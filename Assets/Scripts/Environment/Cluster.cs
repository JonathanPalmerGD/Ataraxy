...
public class Cluster : WorldObject
{
	...
	private AudioSource terrainRumble = null;
	#endregion
	...
	public override void Start()
	{
		...
		transform.FindChild("Cylinder").gameObject.GetComponent<Renderer>().enabled = false;
	...
	{
		Vector3 start = transform.position + Vector3.up * 50;
		Vector3 up = Vector3.up * 0;
		Vector3 halfX = new Vector3(TerrainManager.clusterSize.x / 2, 0, 0);
		Vector3 halfZ = new Vector3(0, 0, TerrainManager.clusterSize.z / 2);
		Debug.DrawLine(start - halfX - halfZ + up, start + halfX - halfZ, Color.blue);
		Debug.DrawLine(start - halfX + halfZ + up, start - halfX - halfZ, Color.blue);
		Debug.DrawLine(start + halfX + halfZ + up, start + halfX - halfZ, Color.blue);
		Debug.DrawLine(start + halfX + halfZ + up, start - halfX + halfZ, Color.blue);
	...
	public void ConfigureClusterNodes()
	{
		...
		//terrainRumble.Stop();
		//Destroy (terrainRumble.gameObject);
	...
	{
		PoissonDiscSampler pds = new PoissonDiscSampler(TerrainManager.clusterSize.x - TerrainManager.clusterSizeEdge, TerrainManager.clusterSize.z - TerrainManager.clusterSizeEdge, 16 + (sizeBonus * 1.2f), poissonKVal);
		...
		foreach (Vector2 sample in pds.Samples())
		{
			...
			if (TerrainManager.Instance.islandPrefabs.Count > 0)
			{
				...
				Vector3 adjustedSize = new Vector3((TerrainManager.clusterSize.x - TerrainManager.clusterSizeEdge) / 2, TerrainManager.clusterSize.y, (TerrainManager.clusterSize.x - TerrainManager.clusterSizeEdge) / 2);
				//Set the new position
				newPosition = new Vector3(sample.x + transform.position.x, transform.position.y + yOffset, sample.y + transform.position.z);
				newPosition -= adjustedSize;
	...
	public void ApplyRandomTexturing(Island island)
	{
		...
		{
			island.GetComponent<Renderer>().material = islandMaterials[Random.Range(0, islandMaterials.Count)];
		}
		else
		{
			/*if (RandomTexture)
			...
			{
				island.GetComponent<Renderer>().material.mainTexture = TerrainManager.Instance.textures[Random.Range(0, TerrainManager.Instance.textures.Count)];
				island.GetComponent<Renderer>().material.mainTextureScale = new Vector2(Random.Range(5, 25), Random.Range(5, 25));
				island.GetComponent<Renderer>().material.color = new Color(Random.Range(.7f, 1f), Random.Range(.7f, 1f), Random.Range(.7f, 1f));
			}
		}
	}
	...
	public void ApplyRandomLandmarkTexturing()
	{
		if (landmarkTerrainObj != null)
		{
			for (int i = 0; i < landmarkTerrainObj.Count; i++)
			{
				if (islandMaterials != null)
				{
					landmarkTerrainObj[i].GetComponent<Renderer>().material = islandMaterials[Random.Range(0, islandMaterials.Count)];
				}
				else
				{
					if (RandomTexture/* && !island.specialIsland*/)
					{
						landmarkTerrainObj[i].GetComponent<Renderer>().material.mainTexture = TerrainManager.Instance.textures[Random.Range(0, TerrainManager.Instance.textures.Count)];
						landmarkTerrainObj[i].GetComponent<Renderer>().material.mainTextureScale = new Vector2(Random.Range(5, 25), Random.Range(5, 25));
						landmarkTerrainObj[i].GetComponent<Renderer>().material.color = new Color(Random.Range(.7f, 1f), Random.Range(.7f, 1f), Random.Range(.7f, 1f));
					}
				}
			}
		}
